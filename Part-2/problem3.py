...
class Graph:
    ...
    def removeUndirectedEdge(self, first: Node, second: Node) -> None:
        if second in self.nodes[first.val].neighbors:
            self.nodes[first.val].neighbors.remove(second)
        if first in self.nodes[second.val].neighbors: 
            self.nodes[second.val].neighbors.remove(first)
...
LL=createLinkedList(100) #0-based indexing
